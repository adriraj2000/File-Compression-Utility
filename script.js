...
window.onload = function () {
	huffman_decodeBtn = document.getElementById("huffman_decode");
	huffman_encodeBtn = document.getElementById("huffman_encode");

	lzw_decodeBtn = document.getElementById("lzw_decode");
	lzw_encodeBtn = document.getElementById("lzw_encode");
	...
	huffman_encodeBtn.onclick = function () {
		var uploadedFile = uploadFile.files[0];
		if (uploadedFile === undefined) {
			alert("No file uploaded.\nPlease upload a file and try again");
			return;
	...
	huffman_decodeBtn.onclick = function (e) {
	...
	// called when compress button is clicked
	lzw_encodeBtn.onclick = function () {
		var uploadedFile = uploadFile.files[0];
		if (uploadedFile === undefined) {
			alert("No file uploaded.\nPlease upload a file and try again");
			return;
	...
	// called when decompress button is clicked
	lzw_decodeBtn.onclick = function () {
		console.log("decode onclick");
		var uploadedFile = uploadFile.files[0];
		if (uploadedFile === undefined) {
			alert("No file uploaded.\nPlease upload a file and try again!");
...
function onclickChanges2(secMsg, word) {
	huffman_decodeBtn.disabled = true;
	huffman_encodeBtn.disabled = true;
	step3.innerHTML = "";
...
function ondownloadChanges(outputMsg) {
	...
	img.src = "./utilities/Green_Tick.png";
	img.id = "doneImg";
